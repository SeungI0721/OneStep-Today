...
- 날씨 좌표는 코드에 고정되어 있어 사용자 위치 기반 날씨는 아닙니다.
...
- Google Places, Google Maps, Google Location 의존성을 제거했습니다.
- Manifest에 직접 들어 있던 Google Maps API Key를 제거했습니다.
- 사용하지 않는 위치 권한과 저장소 권한을 제거했습니다.
- 사용하지 않는 Google Maps 관련 권한과 메타데이터를 제거했습니다.
...
- 2026-07-07 동적 캘린더 변경 후 `activity_statistics.xml` XML 파싱은 정상 확인했습니다.
- 2026-07-07 현재 로컬 Android Studio JBR에서 `lib/jvm.cfg` 파일을 찾지 못해 추가 Gradle 빌드는 실행하지 못했습니다.
