...
명령어는 프로젝트 최상단인 `OneStep-Today/`에서 실행합니다.
...
지도와 날씨 기능을 실제로 사용하려면 프로젝트 최상단의 `local.properties`에 개인 API 키를 추가합니다.
...
- 기상청 단기예보 기반 날씨 표시를 사용하려면 `local.properties`에 `WEATHER_API_KEY`를 추가해야 합니다.
- API 키는 코드가 아니라 `local.properties`로 분리되어 있으며, 공개 저장소에 올리지 않습니다.
